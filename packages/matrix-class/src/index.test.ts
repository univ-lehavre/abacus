...
describe("DenseMatrix.transpose (méthode d'instance)", () => {
  ...
  describe('DenseMatrix.dot (produit matriciel)', () => {
    it('multiplie deux matrices carrées 2x2', () => {
      const A = new DenseMatrix([1, 2, 3, 4], { ncol: 2, nonNegative: false });
      const B = new DenseMatrix([5, 6, 7, 8], { ncol: 2, nonNegative: false });
      const C = A.dot(B);
      expect(C.getData()).toEqual([1 * 5 + 2 * 7, 1 * 6 + 2 * 8, 3 * 5 + 4 * 7, 3 * 6 + 4 * 8]);
      expect(C.nRows).toBe(2);
      expect(C.nCols).toBe(2);
    });

    it('multiplie une matrice 2x3 par une matrice 3x2', () => {
      const A = new DenseMatrix([1, 2, 3, 4, 5, 6], { ncol: 3, nonNegative: false });
      const B = new DenseMatrix([7, 8, 9, 10, 11, 12], { ncol: 2, nonNegative: false });
      const C = A.dot(B);
    ...
    it('lève une erreur si les dimensions sont incompatibles', () => {
      const A = new DenseMatrix([1, 2, 3, 4], { ncol: 2, nonNegative: false });
      const B = new DenseMatrix([5, 6, 7, 8], { ncol: 4, nonNegative: false });
      expect(() => A.dot(B)).toThrow();
    });

    it('multiplie par la matrice identité', () => {
      const A = new DenseMatrix([2, 3, 4, 5], { ncol: 2, nonNegative: false });
      const I = new DenseMatrix([1, 0, 0, 1], { ncol: 2, nonNegative: false });
      const C = A.dot(I);
      expect(C.getData()).toEqual(A.getData());
    });
  });
...
describe('DenseMatrix.eldiv (division élément par élément)', () => {
  it('divise élément par élément deux matrices de mêmes dimensions', () => {
    const A = new DenseMatrix([10, 20, 30, 40], { ncol: 2, nonNegative: false });
    const B = new DenseMatrix([2, 4, 5, 8], { ncol: 2, nonNegative: false });
    const C = A.eldiv(B);
    expect(C.getData()).toEqual([10 / 2, 20 / 4, 30 / 5, 40 / 8]);
    expect(C.nRows).toBe(2);
    expect(C.nCols).toBe(2);
  });

  it('remplace les divisions par zéro par 0 (comportement implémenté)', () => {
    const A = new DenseMatrix([1, 2, 3, 4], { ncol: 2, nonNegative: false });
    const B = new DenseMatrix([1, 0, 0, 2], { ncol: 2, nonNegative: false });
    const C = A.eldiv(B);
    // B[1] et B[2] sont 0 donc résultat 0 selon l'implémentation
    expect(C.getData()).toEqual([1 / 1, 0, 0, 4 / 2]);
  });

  it('lève une erreur si les dimensions sont incompatibles', () => {
    const A = new DenseMatrix([1, 2, 3, 4], { ncol: 2, nonNegative: false });
    const B = new DenseMatrix([1, 2, 3, 4, 5, 6], { ncol: 3, nonNegative: false });
    expect(() => A.eldiv(B)).toThrow();
  });

  it('conserve la non-négativité si les deux matrices sont non-negative', () => {
    const A = new DenseMatrix([4, 6, 8, 10], { ncol: 2, nonNegative: true });
    const B = new DenseMatrix([2, 3, 4, 5], { ncol: 2, nonNegative: true });
    const C = A.eldiv(B);
    expect(C.getData().every(v => v >= 0)).toBe(true);
  });
});
