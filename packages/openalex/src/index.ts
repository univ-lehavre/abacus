...
import { groupMultiSelect, reduceDimensionality } from '@univ-lehavre/strings-clustering';
...
const program = Effect.gen(function* () {
  ...
  // Groupons les affiliations similaires pour faciliter la sélection

  const { docTopicMatrix } = reduceDimensionality(
    raw_affiliations_string,
    final_affiliations.length + 1,
    { minN: 2, maxN: 3 },
  );

  const opts = () => {
    const g = groupMultiSelect(docTopicMatrix, raw_affiliations_string);
    const opts: Record<string, { value: string }[]> = {};
    for (const [key, group] of Object.entries(g)) {
      opts[key] = group.map(name => ({ value: name }));
    }
    return opts;
  };
